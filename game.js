...
// --- SHIFT / RNG HYGIENE ---
const SHIFT_LENGTH = 48;          // End-of-shift after 48 played cards
const RECENT_CARD_LIMIT = 8;      // Prevent streaky repeats
let recentCards = [];             // Stores recent card.text values
...
const GRIND_CARDS = [
    ...
    { text: "Man yelling at a mailbox.", icon: "📫", left: {t:"Welfare Check", e:{trust:5}}, right: {t:"Drive by", e:{safety:2}} },
...
function drawCard(deck) {
    // Avoid repeating the exact same cards too frequently
    let card = null;
    let attempts = 0;
    ...
    do {
        ...
        attempts++;
    } while (recentCards.includes(card.text) && attempts < 20);

    recentCards.push(card.text);
    if (recentCards.length > RECENT_CARD_LIMIT) recentCards.shift();
...
function getNextCard() {
    ...
    // 10% Chance for Boss
    if (roll > 90) return drawCard(BOSS_CARDS);
    // 25% Chance for Heat
    if (roll > 65) return drawCard(HEAT_CARDS);
    // 65% Chance for Grind
    return drawCard(GRIND_CARDS);
}
...
function handleChoice(choice) {
    ...
    // End of shift after 48 played cards
    if (state.turn > SHIFT_LENGTH) {
        updateUI();
        endShift();
        return;
    }
...
function endShift() {
    // Simple end-of-shift break (keeps your current stats, resets the shift counter & repeat buffer)
    const summary =
        "End of shift.\n\n" +
        "Trust: " + state.trust + "/100\n" +
        "Department: " + state.stats + "/100\n" +
        "Safety: " + state.safety + "/100\n" +
        "Budget: " + state.budget + "/100\n\n" +
        "You rack the cruiser. Tomorrow comes fast.";
